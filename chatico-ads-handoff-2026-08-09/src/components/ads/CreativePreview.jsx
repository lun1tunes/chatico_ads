...
/**
 * Размеры превью:
 * - thumb   → компактная квадратная миниатюра для строк списка (drill-down);
 * - default → карточка объявления в списке;
 * - hero    → крупное превью на экране одного объявления.
 */
const VARIANT_CLASS = {
  thumb: 'aspect-square w-14 shrink-0 sm:w-16',
  default: 'aspect-video w-full shrink-0 sm:aspect-[4/3] sm:w-40',
  hero: 'aspect-[4/3] w-full shrink-0 sm:aspect-video sm:w-64',
};

export default function CreativePreview({ id, name, format, variant = 'default', thumbnailUrl }) {
  ...
  if (thumbnailUrl) {
    return (
      ...
      >
        ...
        />
        {!isThumb && (
          <span className="absolute bottom-2 left-2 rounded-md bg-black/60 px-1.5 py-0.5 text-[10px] font-bold text-white">
            {meta.label}
          </span>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      {/* Компактная миниатюра: только иконка формата по центру */}
      {isThumb && (
        <div className="absolute inset-0 flex items-center justify-center">
          {meta.play ? (
            <svg className="ml-0.5 h-5 w-5 text-white drop-shadow" fill="currentColor" viewBox="0 0 24 24">
              <path d="M8 5v14l11-7z" />
            </svg>
          ) : meta.form ? (
            <svg className="h-5 w-5 text-white/90" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h6m-6 4h6M9 8h6M6 4h12v16H6z" />
            </svg>
          ) : meta.carousel ? (
            <svg className="h-5 w-5 text-white/90" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M4 6v12M20 6v12M8 5h8v14H8z" />
            </svg>
          ) : (
            <svg className="h-5 w-5 text-white/90" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.6}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M4 5h16v14H4zM4 16l5-5 4 4 3-3 4 4" />
            </svg>
          )}
        </div>
      )}

      {/* Центральный элемент по типу формата (полное превью) */}
      {!isThumb && (
      <div className="absolute inset-0 flex items-center justify-center">
        {meta.play && (
