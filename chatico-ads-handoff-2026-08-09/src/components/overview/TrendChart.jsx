      ...
      <div className="flex flex-col gap-3 border-b border-outline px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            Динамика расходов и сообщений за период
          </p>
        </div>
        <div className="flex items-center gap-4">
          <LegendDot color="#5E44EB" label="Расход (₸)" />
          <LegendDot color="#8BC53F" label="Обращения" />
        </div>
      </div>
      ...
      <div className="h-72 w-full p-4">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="conversations" name="Обращения" stroke="#8BC53F" strokeWidth={2} fillOpacity={1} fill="url(#colorLeads)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
