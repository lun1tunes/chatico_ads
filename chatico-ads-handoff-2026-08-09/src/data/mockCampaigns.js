...
export const MOCK_CAMPAIGNS = [
  ...
  },
  {
    ...
    shortName: 'Catalog Sales',
    status: 'paused',
  },
];

/** Максимум кампаний в sidebar до пункта «Все кампании». */
export const SIDEBAR_CAMPAIGN_LIMIT = 10;

/** Кампании для sidebar: сначала активные, затем на паузе, не более лимита. */
export function getSidebarCampaigns(limit = SIDEBAR_CAMPAIGN_LIMIT) {
  const active = MOCK_CAMPAIGNS.filter((c) => c.status === 'active');
  const paused = MOCK_CAMPAIGNS.filter((c) => c.status === 'paused');
  return [...active, ...paused].slice(0, limit);
}
...
const CAMPAIGN_DETAILS = {
  camp_001: {
    ...
    currency: 'USD',
    metrics: {
      spent: metric('$50.32', '4%', false, 'Потрачено за период'),
      leads: metric('20 лидов', '12%', true, 'Лиды (форма)'),
      cpl: metric('$2.52', '8%', true, 'Цена за лид'),
      impressions: metric('10 277 раз', '6%', true, 'Показы объявлений'),
    },
    verdict: {
      status: 'good',
      text: '«Кольцевая галерея» — явный лидер: 16 лидов по $2.34. «Чувиха» тоже работает дёшево ($2.00), а вот объявление с Instagram-ссылкой дорогое — $6.90 за лид. Рекомендую перераспределить бюджет в пользу карусели.',
    },
    adSets: [
      {
        id: 'as_001',
        ...
        targeting: {
          ...
          age: '25–45',
          city: 'Алматы',
          audienceType: 'Похожие на клиентов',
          fbDetails: {
            lookalike: true,
            interests: [
              'Малый бизнес',
              ...
              'Предпринимательство',
              'Индивидуальный предприниматель или Бизнес',
            ],
            behaviors: ['Владельцы малых предприятий'],
            jobTitles: [
              'Small business',
              'Business',
              'Owner',
        ...
        ads: [
          {
            id: 'ad_real_1',
            name: 'чувиха',
            thumbnailUrl: '/creatives/ad-chuviha.jpg',
            format: 'video',
            spent: 6.01,
            leads: 3,
            cpl: 2.0,
          ...
          {
            id: 'ad_real_2',
            name: 'https://www.instagram.com/p/DbkNc5QB2XO/',
            thumbnailUrl: '/creatives/ad-instagram.jpg',
            format: 'image',
            spent: 6.9,
            leads: 1,
            cpl: 6.9,
          ...
          {
            id: 'ad_real_3',
            name: 'кольцевая галерея',
            thumbnailUrl: '/creatives/ad-carousel.jpg',
            format: 'carousel',
            spent: 37.41,
            leads: 16,
            cpl: 2.34,
